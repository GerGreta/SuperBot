...
from logic_day import logic_day  # ✅ импорт функции личного дня
...
async def start_handler(message: types.Message):
    await message.answer(
        ...
        "🔹 Или день и месяц рождения (например: 506, 05.06, 2404) — я рассчитаю личный день\n\n"
        "Я рассчитаю:\n"
        "• Число Сознания\n"
        "• Число Действия\n"
        "• Психоматрицу\n"
...
async def message_handler(message: types.Message):
    ...
    # 1️⃣ Сначала проверяем личный день (день 1-2 цифры, месяц 2 цифры)
    personal_day_result = logic_day(text)
    if personal_day_result:
        await message.answer(
            f"🌟 Личный день на сегодня и следующие 8 дней:\n\n{personal_day_result}"
        )
        return

    # 2️⃣ Пробуем распознать полную дату
    ...
    # 4️⃣ Если вообще не распознали
    await message.answer(
        "🤔 Не могу распознать сообщение.\n"
        "Попробуй написать слово, дату рождения или день/месяц рождения."
    )
